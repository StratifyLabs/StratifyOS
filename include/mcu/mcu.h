...
enum {
  MCU_BOARD_CONFIG_FLAG_LED_ACTIVE_HIGH /*! When set, the board LED is active high */ =
    (1 << 0),
  MCU_BOARD_CONFIG_FLAG_SAFE_SVCALL /*! When set, the svcall can only be used by the
                                       kernel */
  = (1 << 1),
  MCU_BOARD_CONFIG_FLAG_ENABLE_CACHE /*! When set, the svcall can only be used by the
                                        kernel */
  = (1 << 2)
};

enum {
  MCU_BOARD_CONFIG_EVENT_ROOT_RESET /*! Event is called after a reset while still in root
                                       mode. Args is null */
  ,
  MCU_BOARD_CONFIG_EVENT_ROOT_FATAL /*! Event is called when a fatal error occurs (called
                                       in priviledged mode).  Args is a c string */
  ,
  MCU_BOARD_CONFIG_EVENT_FATAL /*! Event is called when a fatal error occurs (called in
                                  unpriviledged mode).  Args is a c string */
  ,
  MCU_BOARD_CONFIG_EVENT_CRITICAL /*! Event is called when a critical error occurs (e.g. a
                                     filesystem fails to start).  Args is a c string */
  ,
  MCU_BOARD_CONFIG_EVENT_WARNING /*! Event is called when something isn't quite right but
                                    things should basically still work. Args is a c string
                                  */
  ,
  MCU_BOARD_CONFIG_EVENT_MESSAGE /*! Event is called when a message is available. Args is
                                    a c string */
  ,

  MCU_BOARD_CONFIG_EVENT_ROOT_INITIALIZE_CLOCK /*! Called at the beginning of
                                                  mcu_core_initclock() so that the BSP can
                                                  initialize the clock configuration */
  ,
  MCU_BOARD_CONFIG_EVENT_START_INIT /*! Called with sos_default_thread(). Args is a
                                       pointer to u8 which is the reset type */
  ,
  MCU_BOARD_CONFIG_EVENT_START_FILESYSTEM /*! Called with sos_default_thread() when
                                             filesystem starts. Args is a pointer to u32
                                             which says how many processes started */
  ,
  MCU_BOARD_CONFIG_EVENT_START_LINK /*! Called with sos_default_thread(), when link
                                       starts.  Args is null */
  ,
  ...
  MCU_BOARD_CONFIG_EVENT_SERVICE_CALL_PERMISSION_DENIED /*! MCU tried to execute a service
                                                           call that was not permitted */
  ,

  MCU_BOARD_CONFIG_EVENT_ROOT_WDT_TIMEOUT /*! MCU had a WDT timeout */,
  MCU_BOARD_CONFIG_EVENT_SCHEDULER_IDLE /*! All tasks are idle (this can implement an
                                           alternate sleep implementation) */
  ,
  MCU_BOARD_CONFIG_EVENT_ROOT_DEBUG_INITIALIZED /*! All tasks are idle (this can implement
                                                   an alternate sleep implementation) */
  ,
  MCU_BOARD_CONFIG_EVENT_SYSTEM_ASSERT /*! Software assert() called on process 0 */,
  MCU_BOARD_CONFIG_EVENT_ROOT_CONFIGURE_MPU /*! System MPU has been setup (may be updated
                                               by application) */
  ,
...
typedef struct MCU_PACK {
  ...
  u32 event_handler /*! A callback to an event handler that gets, for example,
                       MCU_BOARD_CONFIG_EVENT_FATAL on a fatal event */
    ;
  u32 arch_config /*! A pointer to MCU architecture specific data, for example,
                     stm32_arch_config_t */
    ;
  u32 secret_key_address /*! A pointer to the secret cryptographic keys to be protected
  ...
  void (*event_handler)(
    ...
    void *) /*! A callback to an event handler that gets, for example,
               MCU_BOARD_CONFIG_EVENT_FATAL on a fatal event */
    ;
  const void *arch_config /*! A pointer to MCU architecture specific data, for example,
                             stm32_arch_config_t */
    ;
  const void *secret_key_address /*! A pointer to the secret cryptographic keys to be
                                    protected from application access. */
    ;
#endif
  u32 secret_key_size /*! The size in bytes of the secret key region (must be MPU
                         compatible). */
    ;
  ...
  u32 os_mpu_text_mask /*! Mask to apply to _text when setting the kernel memory
    ...
    ;
  mcu_pin_t
    led /*! A pin on the board that drives an LED. Use {0xff, 0xff} if not available. */;
  u8 debug_uart_port /*! The port used for the UART debugger. This is only used for _debug
                        builds */
    ;
  u8 resd;
