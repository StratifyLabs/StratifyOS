...
enum {
	WIFI_SCAN_REGION_ASIA,
	WIFI_SCAN_REGION_NORTH_AMERICA
};
...
typedef struct {
	...
	int (*connect)(void * context, const wifi_ssid_info_t * ssid_info, const wifi_auth_info_t * auth);
	int (*disconnect)(void * context);
	int (*start_scan)(
			void * context,
			const wifi_scan_attributes_t * attributes
			);
