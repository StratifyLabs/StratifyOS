...
int link1_transport_slaveread(
  ...
  do {
    ...
    if (link1_transport_wait_start(driver, &pkt, driver->timeout) < 0) {
    ...
    if (link1_transport_wait_packet(driver, &pkt, driver->timeout) < 0) {
      send_ack(driver, LINK_PACKET_NACK, 0);
      driver->wait(driver->timeout);
      driver->flush(driver->handle);
      return -2;
...
int link1_transport_slavewrite(
  ...
  do {
    ...
    // send packet
    if (
      driver->write(driver->handle, &pkt, pkt.size + LINK_PACKET_HEADER_SIZE)
