...
#include "device/dev_ffifo.h"

static int dev_event_write_complete(void * context, const mcu_event_t * event);
static int dev_event_data_ready(void * context, const mcu_event_t * event);

int dev_event_write_complete(void * context, const mcu_event_t * event){
    const devfs_handle_t * handle = context;
    const dev_ffifo_config_t * config = handle->config;
    dev_ffifo_state_t * state = handle->state;
    int nbyte;
...
int dev_event_data_ready(void * context, const mcu_event_t * event){
    const devfs_handle_t * handle = context;
    const dev_ffifo_config_t * config = handle->config;
    dev_ffifo_state_t * state = handle->state;
...
int dev_ffifo_ioctl(const devfs_handle_t * handle, int request, void * ctl){
    const dev_ffifo_config_t * config = handle->config;
    dev_ffifo_state_t * state = handle->state;
    dev_ffifo_info_t * info = ctl;
    mcu_action_t * action = ctl;

    switch(request){

    case I_DEV_FFIFO_GETVERSION: return DEV_FFIFO_VERSION;
    case I_FFIFO_INIT:        
        if( config->tx.buffer ){
            ...
            state->tx.async.handler.callback = dev_event_write_complete;
        ...
        if( config->rx.buffer ){
            ...
            state->rx.async.handler.callback = dev_event_data_ready;
