...
int drive_initialize(const devfs_handle_t *handle) {
  ...
  if (state->is_initialized == 0) {
    ...
    result = config->serial_device->driver.ioctl(
      &config->serial_device->handle, I_QSPI_SETATTR, 0);
    ...
    if (config->qspi_flags & QSPI_FLAG_IS_OPCODE_QUAD) {
      // enter QPI mode
      drive_cfi_qspi_execute_quick_command(handle, config->opcode.enter_qpi_mode, 0, 0);
    }

    if (config->qspi_flags & QSPI_FLAG_IS_ADDRESS_32_BITS) {
      drive_cfi_qspi_execute_quick_command(
        handle, config->opcode.enter_4byte_address_mode, 0, config->qspi_flags);
    }
    ...
    if (config->opcode.write_status != 0 && config->opcode.write_status != 0xff) {
      drive_cfi_qspi_execute_quick_command(
        handle, config->opcode.write_enable, 0, config->qspi_flags);
      u8 update_status = config->opcode.initial_status_value;
      drive_cfi_qspi_execute_command(
        handle, config->opcode.write_status, 0, &update_status, 1, 0, config->qspi_flags);
    }
  }
  ...
  case I_DRIVE_SETATTR: {
    ...
    if (o_flags & DRIVE_FLAG_PROTECT) {
      drive_cfi_qspi_execute_quick_command(
        handle, config->opcode.write_enable, 0, config->qspi_flags);
      drive_cfi_qspi_execute_quick_command(
        handle, config->opcode.protect, 0, config->qspi_flags);
    }
    ...
    if (o_flags & DRIVE_FLAG_UNPROTECT) {
      ...
      drive_cfi_qspi_execute_quick_command(
        handle, config->opcode.unprotect, 0, config->qspi_flags);
    }

    if (o_flags & DRIVE_FLAG_RESET) {
      drive_cfi_qspi_execute_quick_command(
        handle, config->opcode.enable_reset, 0, config->qspi_flags);

      drive_cfi_qspi_execute_quick_command(
        handle, config->opcode.reset, 0, config->qspi_flags);
    }

    if (o_flags & DRIVE_FLAG_POWERUP) {
      drive_cfi_qspi_execute_quick_command(
        handle, config->opcode.power_up, 0, config->qspi_flags);
    }

    if (o_flags & DRIVE_FLAG_POWERDOWN) {
      drive_cfi_qspi_execute_quick_command(
        handle, config->opcode.power_down, 0, config->qspi_flags);
    }
    ...
    if (o_flags & DRIVE_FLAG_ERASE_BLOCKS) {
      ...
      drive_cfi_qspi_execute_quick_command(
        handle, opcode, attr->start + config->info.partition_start,
    ...
    if ((config->opcode.device_erase != 0xff) && (o_flags & DRIVE_FLAG_ERASE_DEVICE)) {
      drive_cfi_qspi_execute_quick_command(
        handle, config->opcode.write_enable, 0, config->qspi_flags);

      drive_cfi_qspi_execute_quick_command(
        handle, config->opcode.device_erase, 0, config->qspi_flags);
    }

  } break;

  case I_DRIVE_GETINFO:
    info->o_flags = DRIVE_FLAG_INIT | DRIVE_FLAG_RESET | DRIVE_FLAG_PROTECT
                    | DRIVE_FLAG_UNPROTECT | DRIVE_FLAG_ERASE_BLOCKS | DRIVE_FLAG_POWERUP
                    | DRIVE_FLAG_POWERDOWN | 0;
    ...
    info->addressable_size =
      config->info.addressable_size; // one byte for each address location
    info->write_block_size = config->info.write_block_size; // can write one byte at a
                                                            // time
...
int drive_cfi_qspi_read(const devfs_handle_t *handle, devfs_async_t *async) {
  ...
  int result = drive_cfi_qspi_execute_command(
    handle, config->opcode.fast_read, config->opcode.read_dummy_cycles,
...
int drive_cfi_qspi_write(const devfs_handle_t *handle, devfs_async_t *async) {
  ...
  int result = drive_cfi_qspi_execute_command(
    handle, config->opcode.page_program, config->opcode.write_dummy_cycles,
    0, // no data
    async->nbyte, async->loc + config->info.partition_start,
    config->qspi_flags | QSPI_FLAG_IS_ADDRESS_WRITE);
...
int drive_cfi_qspi_execute_command(
  ...
  int result = config->serial_device->driver.ioctl(
    &config->serial_device->handle, I_QSPI_EXECCOMMAND, &command);
