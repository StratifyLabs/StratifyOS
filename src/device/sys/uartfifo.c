...
static int set_read_action(const devfs_handle_t * cfg, mcu_callback_t callback){
	...
	action.prio = 0;
	if( mcu_uart_setaction(cfg->port, &action) < 0 ){
		return -1;
	}
...
static int data_received(void * context, mcu_event_t * data){
	...
	while( mcu_uart_get(cfg->port, &c) == 0 ){
...
int uartfifo_ioctl(const devfs_handle_t * cfg, int request, void * ctl){
	...
	const uartfifo_config_t * cfgp = cfg->config;
	uartfifo_state_t * state = cfg->state;
	int ret;
	switch(request){
	case I_FIFO_GETINFO:
	...
	case I_FIFO_FLUSH:
		fifo_flush(&(state->fifo));
		return mcu_uart_flush(cfg->port, NULL);
		break;
	case I_UART_SETATTR:
		if(  (ret = mcu_uart_setattr(cfg->port, ctl)) < 0 ){
			return ret;
		}
		/* no break */
	case I_FIFO_INIT:
		...
		break;
	default:
		return mcu_uart_ioctl(cfg, request, ctl);
}
return 0;
}
