...
void ffifo_data_received(const ffifo_config_t *handle, ffifo_state_t *state) {
  if (state->transfer_handler.read != NULL) {
    int bytes_read;
    if (
      (bytes_read = ffifo_read_buffer(
         handle, state, state->transfer_handler.read->buf,
...
void ffifo_cancel_async_read(ffifo_state_t *state) {
  devfs_execute_read_handler(
    &state->transfer_handler, 0, SYSFS_SET_RETURN(EIO), MCU_EVENT_FLAG_CANCELED);
}
  ...
  if (state->transfer_handler.write != NULL) {
    ...
    if (
      (bytes_written = ffifo_write_buffer(
         config, state, state->transfer_handler.write->buf_const,
...
int ffifo_ioctl_local(
  ...
  case I_MCU_SETACTION:
    ...
    // ffifo doesn't store a local handler so it can't set an arbitrary action on
    // read/write
  ...
  if ((async->nbyte % config->frame_size) != 0) {
    ...
    if (bytes_read == 0) {
      if (
        ...
        || (state->atomic_position.access.tail == config->frame_count)) {
