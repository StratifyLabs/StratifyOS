...
int event_write_complete(void *context, const mcu_event_t *event) {
  ...
  if (state->tx.async.result < 0) {
    state->tx.error = state->tx.async.result;
    sos_debug_log_error(
      SOS_DEBUG_DEVICE, "error: %s():%d (%d,%d)", __FUNCTION__, __LINE__,
      SYSFS_GET_RETURN(state->tx.async.result),
      SYSFS_GET_RETURN_ERRNO(state->tx.async.result));
    return 0;
  }
  ...
  // check for underflow
  if (
    ffifo_state->atomic_position.access.tail
    != config->tx.frame_count) { // buffer should be full when this event fires -- if not
                                 // fill it with zeros
    ...
    while (ffifo_state->atomic_position.access.tail != config->tx.frame_count) {
      // if buffer is not full -- make it full of zeros -- what happens if application is
      // writing while this write -- interrupt priority?
...
int event_data_ready(void *context, const mcu_event_t *event) {
  ...
  if ((state->rx.async.result < 0) || (o_events & MCU_EVENT_FLAG_CANCELED)) {
    state->rx.error = state->rx.async.result;
  ...
  if (
    ffifo_state->atomic_position.access.tail
    != ffifo_state->atomic_position.access.head) { // buffer should be empty
    if (ffifo_state->o_flags & FIFO_FLAG_IS_READ_BUSY) {
      ffifo_state->o_flags |= FIFO_FLAG_IS_WRITE_WHILE_READ_BUSY;
    }
    ffifo_state->o_flags |= FIFO_FLAG_IS_OVERFLOW;
    ffifo_state->atomic_position.access.tail =
      ffifo_state->atomic_position.access.head; // forces the buffer to be empty
  }
...
int stream_ffifo_open(const devfs_handle_t *handle) {
  ...
  // is device a streaming device? --does it support MCU_EVENT_FLAG_HIGH and
  // MCU_EVENT_FLAG_LOW
...
int stream_ffifo_ioctl(const devfs_handle_t *handle, int request, void *ctl) {
  ...
  case I_STREAM_FFIFO_GETVERSION:
    return STREAM_FFIFO_VERSION;
  case I_STREAM_FFIFO_SETATTR:
    ...
    if (attr->o_flags & STREAM_FFIFO_FLAG_START) {
      ...
      if (config->rx.buffer) {
        // the application reads the RX buffer the is written by the device and data that
      ...
      if (config->tx.buffer) {
        // The application writes data to the FIFO that is then read by the hardware and
        // written to a device
        ...
        state->tx.async.result = 0;
        state->tx.async.handler.context = (void *)handle;
        state->tx.async.handler.callback = event_write_complete;

        // write block is true but the function doesn't block -- returns EGAIN if it is
        // written while full
        ...
        // There needs to be a for loop here to inc the head to the halfway point in case
        // tx.frame_count != 2
        u32 i;
  ...
  case I_STREAM_FFIFO_GETINFO:
    ...
    info->o_flags =
      STREAM_FFIFO_FLAG_FLUSH | STREAM_FFIFO_FLAG_START | STREAM_FFIFO_FLAG_STOP;
  ...
  case I_MCU_SETACTION:
    ...
    if (
      (action->o_events & MCU_EVENT_FLAG_DATA_READY) && (action->handler.callback == 0)) {
      // cancel the ffifo rx
