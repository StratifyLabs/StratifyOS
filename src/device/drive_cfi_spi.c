...
int drive_initialize(const devfs_handle_t *handle) {
  ...
  if (state->is_initialized == 0) {
    ...
    result = config->serial_device->driver.ioctl(
      &config->serial_device->handle, I_SPI_SETATTR, 0);
    ...
    if (config->opcode.write_status != 0 && config->opcode.write_status != 0xff) {
      ...
      drive_cfi_spi_write_instruction_with_cs(
        handle, config->opcode.write_status, &update_status, 1);
    }
  }
...
int drive_cfi_spi_ioctl(const devfs_handle_t *handle, int request, void *ctl) {
  ...
  case I_DRIVE_SETATTR: {
    ...
    if (o_flags & DRIVE_FLAG_ERASE_BLOCKS) {
      ...
      drive_cfi_spi_write_instruction_with_cs(handle, config->opcode.write_enable, 0, 0);
      drive_cfi_spi_write_instruction_with_cs(
        handle, config->opcode.block_erase, address, sizeof(address));
  ...
  case I_DRIVE_GETINFO:
    info->o_flags = DRIVE_FLAG_INIT | DRIVE_FLAG_RESET | DRIVE_FLAG_PROTECT
                    | DRIVE_FLAG_UNPROTECT | DRIVE_FLAG_ERASE_BLOCKS
                    | DRIVE_FLAG_ERASE_DEVICE | DRIVE_FLAG_POWERUP | DRIVE_FLAG_POWERDOWN
                    | 0;
...
int drive_cfi_spi_handle_complete(void *context, const mcu_event_t *event) {
  ...
  devfs_execute_event_handler(
    &state->handler, MCU_EVENT_FLAG_WRITE_COMPLETE | MCU_EVENT_FLAG_DATA_READY, 0);
...
int drive_cfi_spi_read(const devfs_handle_t *handle, devfs_async_t *async) {
  ...
  drive_cfi_spi_write_instruction(
    handle, config->opcode.fast_read, fast_read, sizeof(fast_read));
...
int drive_cfi_spi_write(const devfs_handle_t *handle, devfs_async_t *async) {
  ...
  drive_cfi_spi_write_instruction(
    handle, config->opcode.page_program, page_program, sizeof(page_program));
...
u8 drive_cfi_spi_read_status_with_cs(const devfs_handle_t *handle) {
  ...
  u8 status = 0xff;
  drive_cfi_spi_write_instruction_with_cs(
    handle, config->opcode.read_busy_status, &status, 1);
  return status;
}
...
int drive_cfi_spi_write_instruction(
  ...
  config->serial_device->driver.ioctl(
    &config->serial_device->handle, I_SPI_SWAP, (void *)(u32)instruction);
  for (u8 i = 0; i < data_size; i++) {
    data[i] = (u8)config->serial_device->driver.ioctl(
      &config->serial_device->handle, I_SPI_SWAP, (void *)(u32)data[i]);
  }
