...
void sst25vf_share_write_opcode_addr(const device_cfg_t * cfg, uint8_t opcode, uint32_t addr){
	...
	addrp = (uint8_t*)&addr;
	//send the opcode
	mcu_spi_swap(cfg->periph.port, (void*)(ssize_t)opcode);
	//send the 3-byte address MSB to LSB (assumes little-endian arch)
	mcu_spi_swap(cfg->periph.port, (void*)(ssize_t)addrp[2]);
	mcu_spi_swap(cfg->periph.port, (void*)(ssize_t)addrp[1]);
	mcu_spi_swap(cfg->periph.port, (void*)(ssize_t)addrp[0]);
}
...
void sst25vf_share_write_byte(const device_cfg_t * cfg, uint32_t addr, char byte){
	...
	mcu_spi_swap(cfg->periph.port, (void*)(ssize_t)byte);
	sst25vf_share_deassert_cs(cfg);
}
...
char sst25vf_share_read_byte(const device_cfg_t * cfg, uint32_t addr){
	...
	mcu_spi_swap(cfg->periph.port, 0); //dummy byte output
	byte = mcu_spi_swap(cfg->periph.port, (void*)(ssize_t)0xFF);
	sst25vf_share_deassert_cs(cfg);
	return byte;
}
...
uint8_t sst25vf_share_read_status(const device_cfg_t * cfg){
	uint8_t status;
	sst25vf_share_assert_cs(cfg);
	mcu_spi_swap(cfg->periph.port, (void*)SST25VF_INS_RD_STATUS);
	status = mcu_spi_swap(cfg->periph.port, (void*)(ssize_t)0xFF);
	sst25vf_share_deassert_cs(cfg);
	return status;
}
...
void sst25vf_share_write_status(const device_cfg_t * cfg, uint8_t status){
	sst25vf_share_write_enable(cfg);
	sst25vf_share_assert_cs(cfg);
	mcu_spi_swap(cfg->periph.port, (void*)(ssize_t)SST25VF_INS_WR_STATUS);
	mcu_spi_swap(cfg->periph.port, (void*)(ssize_t)status);
	sst25vf_share_deassert_cs(cfg);
}
...
void sst25vf_share_read_id(const device_cfg_t * cfg, char * dest){
	int i;
	sst25vf_share_assert_cs(cfg);
	mcu_spi_swap(cfg->periph.port, (void*)(ssize_t)SST25VF_INS_RD_ID);
	for(i=0; i < 4; i++){
		dest[i] = mcu_spi_swap(cfg->periph.port, (void*)(ssize_t)0xFF);
	}
	sst25vf_share_deassert_cs(cfg);
}
