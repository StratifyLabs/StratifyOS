...
static int
erase_blocks(const devfs_handle_t *handle, uint32_t block_num, uint32_t end_block);
...
static drive_sdspi_r1_t
exec_cmd_r1(const devfs_handle_t *handle, uint8_t cmd, uint32_t arg, uint8_t *r);
// static drive_sdspi_r2_t _drive_sdspi_cmd_r2(const devfs_handle_t * handle, uint8_t cmd,
// uint32_t arg, uint8_t * r);
static drive_sdspi_r3_t
exec_cmd_r3(const devfs_handle_t *handle, uint8_t cmd, uint32_t arg);

static int
send_cmd(const devfs_handle_t *handle, uint8_t cmd, uint32_t arg, uint8_t *response);
static int parse_response(uint8_t *response, int num, drive_sdspi_r_t *r, uint32_t *arg);
static int
parse_data(uint8_t *dest, int nbyte, int count, uint8_t token, uint8_t *response);
...
// static int _drive_sdspi_write_data(const devfs_handle_t * handle, const void * data,
// int nbyte);
  ...
  case I_DRIVE_SETATTR:
    ...
    if (o_flags & DRIVE_FLAG_INIT) {
      ...
      do {
        ...
        resp.r1 = exec_cmd_r1(
          handle, SDSPI_ACMD41_SD_SEND_OP_COND, 1 << 30,
  ...
  case I_DRIVE_GETINFO:
    ...
    if (is_sdsc(handle)) {
      ...
      info->num_write_blocks =
        (c_size + 1) * 1024 * 512
        / BLOCK_SIZE; // capacity = (C_SIZE+1)*512KByte -- block is capacity / BLOCK_SIZE
    }
    ...
    if (erase_size == 0) {
      ...
      info->erase_device_time = info->erase_block_time * info->num_write_blocks
...
int drive_sdspi_close(const devfs_handle_t *handle) { return 0; }
...
int get_status(const devfs_handle_t *handle, uint8_t *buf) {
  ...
  // now read the data
  ret =
    read_data(handle, buf, sizeof(_drive_sdspi_status_t), SDSPI_START_BLOCK_TOKEN, tmp);
  if (ret < 0) {
    return SYSFS_SET_RETURN(EIO);
  }
...
/*
drive_sdspi_r2_t _drive_sdspi_cmd_r2(const devfs_handle_t * handle, uint8_t cmd, uint32_t
arg, uint8_t * r){ uint8_t tmp[CMD_FRAME_SIZE]; uint8_t * response; if( r == 0 ){ response
= tmp; } else { response = r;
         }
         drive_sdspi_r_t ret;
