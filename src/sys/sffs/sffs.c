...
int sffs_init(const void * cfg){
	...
	if ( err == -1 ){
		...
		if( sffs_mkfs(cfg) < 0 ){
			...
			mcu_debug_log_info(MCU_DEBUG_FILESYSTEM, "Format complete");
		}
	...
	mcu_debug_log_info(MCU_DEBUG_FILESYSTEM, "Found %d bad files", bad_files);
...
int sffs_mkfs(const void * cfg){
	...
	mcu_debug_log_info(MCU_DEBUG_FILESYSTEM, "Erase device");
	if ( (ret = sffs_dev_erase(cfg)) < 0 ){
		...
		mcu_debug_log_info(MCU_DEBUG_FILESYSTEM, "Init serial number");
...
int sffs_unlink(const void * cfg, const char * path){
	...
	mcu_debug_log_info(MCU_DEBUG_FILESYSTEM, "unlink serialno %d", entry.serialno);
...
int sffs_open(const void * cfg, void ** handle, const char * path, int flags, int mode){
	...
	mcu_debug_log_info(MCU_DEBUG_FILESYSTEM, "Open serialno %d (%d)", entry.serialno, err);
	...
	if ( err == SFFS_DIR_PATH_EXISTS ){
		...
		if ( (flags & O_EXCL) && (flags & O_CREAT) ){
			...
			mcu_debug_log_info(MCU_DEBUG_FILESYSTEM, "create new file (old file truncated on close)");
		...
		if ( flags & (O_CREAT) ){
			...
			mcu_debug_log_info(MCU_DEBUG_FILESYSTEM, "file %s does not exist", path);
			ret = SYSFS_SET_RETURN(ENOENT);
		}
	} else {
		//Path does not exist
		mcu_debug_log_info(MCU_DEBUG_FILESYSTEM, "file %s does not exist here", path);
		ret = SYSFS_SET_RETURN(ENOENT);
	}
