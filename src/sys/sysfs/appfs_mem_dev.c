...
#define DECLARE_APPFS_CONFIG_STATE()                                                     \
  const appfs_mem_config_t *config = handle->config;                                     \
  appfs_mem_state_t *state = handle->state

#define INVALID_PAGE ((u32)-1)
static const appfs_mem_section_t *
get_page_section_info(const appfs_mem_config_t *config, u32 page, u32 type, u32 *offset);
...
static int
get_page_info(const appfs_mem_config_t *config, u32 page, u32 type, mem_pageinfo_t *info);
...
int appfs_mem_ioctl(const devfs_handle_t *handle, int request, void *ctl) {
  switch (request) {
  case I_MEM_GETVERSION:
    return MEM_VERSION;
  case I_MEM_GETINFO:
    return appfs_mem_getinfo(handle, ctl);
  case I_MEM_SETATTR:
    return appfs_mem_setattr(handle, ctl);
  case I_MEM_SETACTION:
    return appfs_mem_setaction(handle, ctl);
  case I_MEM_ERASEPAGE:
    return appfs_mem_erasepage(handle, ctl);
  case I_MEM_GETPAGEINFO:
    return appfs_mem_getpageinfo(handle, ctl);
  case I_MEM_WRITEPAGE:
    return appfs_mem_writepage(handle, ctl);
  }
  return SYSFS_SET_RETURN(EINVAL);
}
...
int appfs_mem_getinfo(const devfs_handle_t *handle, void *ctl) {
  ...
  info->external_ram_pages =
    get_memory_page_count(config, MEM_FLAG_IS_RAM | MEM_FLAG_IS_EXTERNAL);
  info->external_ram_size =
    get_memory_size(config, MEM_FLAG_IS_RAM | MEM_FLAG_IS_EXTERNAL);
  info->tightlycoupled_ram_pages =
...
int appfs_mem_getpageinfo(const devfs_handle_t *handle, void *ctl) {
  ...
  if (pageinfo->o_flags & MEM_FLAG_IS_QUERY) {
    ...
    // pageinfo->size = get_page_size(config, pageinfo->num, type &
    // (MEM_FLAG_IS_RAM|MEM_FLAG_IS_FLASH));
    if (type == 0) {
      return SYSFS_SET_RETURN(EINVAL);
    }
    // return 0;
...
int appfs_mem_writepage(const devfs_handle_t *handle, void *ctl) {
  ...
  if (type & MEM_FLAG_IS_RAM) {
    ...
    if (config->flash_driver != 0) {
      result = config->flash_driver->driver.ioctl(
        &config->flash_driver->handle, I_FLASH_WRITEPAGE, ctl);
    } else {
      return SYSFS_SET_RETURN(ENOTSUP);
    }
  }
...
const appfs_mem_section_t *
get_page_section_info(const appfs_mem_config_t *config, u32 page, u32 type, u32 *offset) {
  u32 find_page = 0;
  for (u32 i = 0; i < config->section_count; i++) {
    if (config->sections[i].o_flags & type) { // does page match any flag in type -- ram
                                              // types will have same numbering
  ...
  *type = 0; // zero means it didn't find anything
  for (u32 i = 0; i < config->section_count; i++) {
    ...
    if( (address >= config->sections[i].address) &&
      ...
      u32 offset =
        (address - config->sections[i].address) / config->sections[i].page_size;
