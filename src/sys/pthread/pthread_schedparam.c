...
/*! \details This function gets \a thread's scheduling policy and scheduling parameters
 * and stores them in \a policy and \a param respectively.
 *
...
int pthread_setschedparam(pthread_t thread, int policy, struct sched_param *param) {
  ...
  if (
    ((uint8_t)param->sched_priority >= min_prio)
