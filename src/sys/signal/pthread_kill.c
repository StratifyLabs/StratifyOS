...
/*! \cond */
static int
signal_root_forward(int send_tid, int tid, int si_signo, int si_sigcode, int sig_value)
  MCU_ROOT_CODE;
static void signal_forward_handler(int send_tid, int signo, int sigcode, int sigvalue);

// this checks to see if sending a signal will cause a stack/heap collision in the target
// thread
...
int devfs_signal_callback(void *context, const mcu_event_t *data) {
  // This only works if the other parts of the interrupt handler have not modified the
  // stack
  devfs_signal_callback_t *args = (devfs_signal_callback_t *)context;
  if (
    signal_root_send(0, args->tid, args->si_signo, args->si_sigcode, args->sig_value, 1)
    < 0) {
    // return 0; //this will dis-regard the callback so additional events stop sending
    // signals
  }
...
int signal_root_send(
  ...
  if (((tid == task_get_current())) && (forward != 0)) {
    // If the receiving tid is currently executing, sending the signal directly will
    // corrupt the stack So we stack a signal on task 0 and have it send the signal This
    // only happens when root signals are sent because the stack is in an unknown state
...
int signal_send(int tid, int si_signo, int si_sigcode, int sig_value) {
  ...
  if (si_signo != 0) {
    ...
    if (delay_mutex != NULL) {
      ...
      if (pthread_mutex_timedlock(delay_mutex, &abstime) == 0) { // wait until the task
                                                                 // releases the mutex
