 ...
 * @{
 *
 * \details This interface accesses directory entries (both files and folders).  Here is
 * an example of how to use this interface.
 *
 * \code
 * #include <dirent.h>
 * #include <stdio.h>
 ...
 * - ENAMETOOLONG: \a dirname exceeds \a PATH_MAX or an element of \a dirname exceeds \a
 * NAME_MAX
 *
 */
 ...
 */
int readdir_r(
  DIR *dirp /*! a pointer to the directory structure */,
  struct dirent *entry /*! a pointer to the destination memory */,
  struct dirent *
    *result /*! this value is assigned to \a entry on success and NULL on failure */) {
  int err;
  const sysfs_t *fs;
  if (check_ebadf(dirp) < 0) {
    if (result) {
