...
int link_readserialno(link_transport_mdriver_t *driver, char *serialno, int len) {
  ...
  link_debug(
    LINK_DEBUG_INFO, "Send command (%d) to read serial number on %p", op.cmd,
  ...
  link_debug(
    LINK_DEBUG_MESSAGE, "Read the serial number (%d of %d bytes)", reply.err, len);
  if (reply.err > 0) {
...
int link_mkfs(link_transport_mdriver_t *driver, const char *path) {
  ...
  if (reply.err < 0) {
    ...
    if (link_errno == 16) {
      link_debug(
        LINK_DEBUG_INFO, "Format started but device may be busy while it completes");
    }
  }
  return reply.err;
        ...
        if (link_isbootloader(driver)) {
          ...
          if (sys_fd >= 0) {
            if (link_ioctl(driver, sys_fd, I_SYS_GETINFO, &sys_info) == 0) {
              ...
              sprintf(
                entry, "%s:%s:%s", sys_info.name, sys_info.kernel_version, serialno);
            }
 ...
 */
void link_del_device_list(char *sn_list) { free(sn_list); }
