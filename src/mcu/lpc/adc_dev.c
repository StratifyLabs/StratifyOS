...
static void exec_callback(void * data) MCU_PRIV_CODE;
...
void _mcu_adc_dev_power_off(int port){
	if ( adc_local.ref_count > 0 ){
		if ( adc_local.ref_count == 1 ){
			LPC_ADC->CR = 0; //reset the control -- clear the PDN bit
...
int _mcu_adc_dev_read(const device_cfg_t * cfg, device_transfer_t * rop){
	...
	adc_local.handler.callback = rop->callback;
	adc_local.handler.context = rop->context;
...
void exec_callback(void * data){
	...
	_mcu_core_exec_event_handler(&(adc_local.handler), data);
}
...
void _mcu_core_adc_isr(void){
	if ( adc_local.len > 0 ){
		*adc_local.bufp++ = (adc_sample_t)(LPC_ADC->GDR & 0xFFFF);
...
int mcu_adc_setaction(int port, void * ctl){
	...
	adc_local.handler.callback = action->callback;
	adc_local.handler.context = action->context;
