...
static dac_local_t dac_local MCU_SYS_MEM;
...
int mcu_dac_setaction(int port, void * ctl){
	...
	dac_local.handler.callback = action->callback;
	dac_local.handler.context = action->context;
...
int _mcu_dac_dev_write(const device_cfg_t * cfg, device_transfer_t * wop){
	...
	dac_local.handler.callback = wop->callback;
	dac_local.handler.context = wop->context;
...
void exec_callback(void * data){
	...
	LPC_DAC->CTRL = 0;
	_mcu_core_exec_event_handler(&(dac_local.handler), 0);
}
