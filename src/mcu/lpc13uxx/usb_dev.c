...
typedef struct {
	mcu_callback_t write_callback[DEV_USB_LOGICAL_ENDPOINT_COUNT];
	mcu_callback_t read_callback[DEV_USB_LOGICAL_ENDPOINT_COUNT];
	void * write_context[DEV_USB_LOGICAL_ENDPOINT_COUNT];
	void * read_context[DEV_USB_LOGICAL_ENDPOINT_COUNT];
...
void clear_callbacks(){
	memset(usb_local.write_callback, 0, DEV_USB_LOGICAL_ENDPOINT_COUNT * sizeof(void(*)(int)));
	memset(usb_local.read_callback, 0, DEV_USB_LOGICAL_ENDPOINT_COUNT * sizeof(void(*)(int)));
}
...
void _mcu_usb_dev_power_off(int port){
	if ( usb_local.ref_count > 0 ){
		if ( usb_local.ref_count == 1 ){
			_mcu_cortexm_priv_disable_irq((void*)(USB_IRQ_IRQn));  //Enable the USB interrupt
...
int mcu_usb_setattr(int port, void * ctl){
	...
	_mcu_cortexm_priv_enable_irq((void*)USB_IRQ_IRQn);  //Enable the USB interrupt
...
int mcu_usb_setaction(int port, void * ctl){
	...
	if ( (log_ep < DEV_USB_LOGICAL_ENDPOINT_COUNT)  ){
		if( action->o_events == USB_EVENT_DATA_READY ){
			...
			usb_local.read_callback[ log_ep ] = action->handler.callback;
			usb_local.read_context[ log_ep ] = action->handler.context;
			return 0;
		} else if( action->o_events == USB_EVENT_WRITE_COMPLETE ){
			usb_local.write_callback[ log_ep ] = action->handler.callback;
			usb_local.write_context[ log_ep ] = action->handler.context;
			return 0;
		}
	}
...
int _mcu_usb_dev_read(const devfs_handle_t * cfg, devfs_async_t * rop){
	...
	if ( usb_local.read_ready & (1<<loc) ){
		...
		if ( !(rop->flags & O_NONBLOCK) ){
			...
			usb_local.read_callback[loc] = rop->handler.callback;
			usb_local.read_context[loc] = rop->handler.context;
			ret = 0;
		} else {
			errno = EAGAIN;
			ret = -1;
...
int _mcu_usb_dev_write(const devfs_handle_t * cfg, devfs_async_t * wop){
	...
	usb_local.write_callback[ep] = wop->handler.callback;
	usb_local.write_context[ep] = wop->handler.context;
