...
int _mcu_pio_set_event(int port, int event, int pin){
	...
	if( port == 0 ){
		LPC_SYSCON->SYSAHBCLKCTRL |= (1<<23);
		LPC_GPIO_GROUP_INT0->CTRL = 0;
		LPC_GPIO_GROUP_INT0->PORT_POL[0] &= ~(1<<pin);
		LPC_GPIO_GROUP_INT0->PORT_POL[0] |= polarity;
		if( event != PIO_ACTION_EVENT_NONE ){
			LPC_GPIO_GROUP_INT0->PORT_ENA[0] |= (1<<pin);
		}
		LPC_SYSCON->STARTERP1 |= (1<<20); //enable wakeup on group 0
	} else {
		LPC_SYSCON->SYSAHBCLKCTRL |= (1<<24);
		LPC_GPIO_GROUP_INT1->CTRL = 0;
		LPC_GPIO_GROUP_INT1->PORT_POL[1] &= ~(1<<pin);
		LPC_GPIO_GROUP_INT1->PORT_POL[1] |= polarity;
		if( event != PIO_ACTION_EVENT_NONE ){
			LPC_GPIO_GROUP_INT1->PORT_ENA[1] |= (1<<pin);
		}
		LPC_SYSCON->STARTERP1 |= (1<<21); //enable wakeup on group 1
	}
