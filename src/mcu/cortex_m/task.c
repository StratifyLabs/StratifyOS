...
int task_init(int interval,
	...
	//enable use of PSP
	mcu_cortexm_priv_set_thread_stack_ptr( (void*)task_table[0].sp );
	task_current = 0;
...
int task_new_thread(void *(*p)(void*),
	...
	mcu_core_privcall( (core_privcall_t)task_priv_new_task, &task);
	tid = task.tid;
			...
			if ( (task_table[i].reent == reent_ptr) && (task_table[i].global_reent == reent_ptr) ){
				...
				if ( task_used_asserted(i) ){
					if ( (i == task_get_current()) ){
						...
						mcu_core_privcall(mcu_cortexm_priv_get_thread_stack_ptr, &stackaddr);
						return stackaddr;
					} else {
...
uint64_t task_gettime(int tid){
	...
	if ( tid != task_get_current() ){
		...
		mcu_core_privcall((core_privcall_t)priv_task_tmr_rd, &val);
		return task_table[tid].timer.t + val;
	}
}
...
void task_context_switcher(){
	...
	if ( task_isfifo_asserted(task_current) ){
		...
		mcu_cortexm_disable_systick_irq();
	} else {
		...
		mcu_cortexm_enable_systick_irq();
	}
...
int task_interrupt(task_interrupt_t * intr){
	if ( intr->tid < task_get_total() ){
		mcu_core_privcall(task_priv_interrupt, intr);
		return 0;
	}
	return -1;
}
