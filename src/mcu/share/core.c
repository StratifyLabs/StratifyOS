...
int mcu_core_ioctl(const device_cfg_t * cfg, int request, void * ctl){
...
int mcu_core_read(const device_cfg_t * cfg, device_transfer_t * rop){
	errno = ENOTSUP;
	return -1;
}


int mcu_core_write(const device_cfg_t * cfg, device_transfer_t * wop){
	errno = ENOTSUP;
	return -1;
}
