



if( ${SOS_BSP_ARCH} STREQUAL armv7-m )
	set(BUILD_FLOAT ${TOOLCHAIN_FLOAT_DIR_ARMV7M})
	set(BUILD_LIBRARY_DIR ${TOOLCHAIN_INSTALL_DIR_ARMV7M})
	set(BUILD_GCC_LIB gcc)
	set(BUILD_FLOAT_OPTIONS "")
elseif( ${SOS_BSP_ARCH} STREQUAL armv7e-m )
	set(BUILD_FLOAT ${TOOLCHAIN_FLOAT_DIR_ARMV7EM_FPU})
	set(BUILD_LIBRARY_DIR ${TOOLCHAIN_INSTALL_DIR_ARMV7EM_FPU})
	set(BUILD_FLOAT_OPTIONS ${TOOLCHAIN_FLOAT_OPTIONS_ARMV7EM_FPU})
	set(BUILD_GCC_LIB gcc-hard)
endif()

if( ${SOS_BSP_CONFIG} STREQUAL release )
	set(BUILD_TYPE "")
	set(BUILD_NAME build_release)
	set(BUILD_UNDEFINE_SYMBOL symbols_table)
	set(BUILD_HARDWARD_ID ,--defsym=mcu_core_hardware_id=${SOS_BSP_HARDWARD_ID})
	set(BUILD_EXTRA_LIBRARIES "")
elseif( ${SOS_BSP_CONFIG} STREQUAL debug )
	set(BUILD_TYPE "_debug")
	set(BUILD_NAME build_debug)
	set(BUILD_UNDEFINE_SYMBOL symbols_table)
	set(BUILD_EXTRA_LIBRARIES "")
	set(BUILD_HARDWARD_ID ,--defsym=mcu_core_hardware_id=${SOS_BSP_HARDWARD_ID})
elseif( ${SOS_BSP_CONFIG} STREQUAL release_boot )
	set(BUILD_TYPE "")
	set(BUILD_NAME build_release_boot)
	set(BUILD_EXTRA_LIBRARIES -lsos_boot)
	set(BUILD_UNDEFINE_SYMBOL _main)
	set(BUILD_HARDWARD_ID "")
elseif( ${SOS_BSP_CONFIG} STREQUAL debug_boot )
	set(BUILD_TYPE "_debug")
	set(BUILD_NAME build_debug_boot)
	set(BUILD_UNDEFINE_SYMBOL _main)
	set(BUILD_EXTRA_LIBRARIES -lsos_boot_debug)
	set(BUILD_HARDWARD_ID "")
endif()

set(CMAKE_RUNTIME_OUTPUT_DIRECTORY ${CMAKE_SOURCE_DIR}/${BUILD_NAME} CACHE INTERNAL "runtime output")

file(MAKE_DIRECTORY ${CMAKE_RUNTIME_OUTPUT_DIRECTORY})

if(SOS_BSP_OPTIMIZATION)
	set(BUILD_OPTIMIZATION ${SOS_BSP_OPTIMIZATION})
else()
	set(BUILD_OPTIMIZATION "-Os")
endif()

if(SOS_BSP_LIBRARIES)
	set(BUILD_LIBRARIES "${SOS_BSP_LIBRARIES} -lsos_sys${BUILD_TYPE} -lsos_mcu_${SOS_BSP_DEVICE_FAMILY}${BUILD_TYPE} -lm -lc -lsos_sys${BUILD_TYPE} -l${BUILD_GCC_LIB}")
else()
	set(BUILD_LIBRARIES "-lsos_sys${BUILD_TYPE} -lsos_mcu_${SOS_BSP_DEVICE_FAMILY}${BUILD_TYPE} -lm -lc -lsos_sys${BUILD_TYPE} -l${BUILD_GCC_LIB}")
endif()
set(BUILD_FLAGS -march=${SOS_BSP_ARCH} ${BUILD_FLOAT_OPTIONS} ${BUILD_OPTIMIZATION})
set(LINKER_FLAGS "-L${TOOLCHAIN_LIB_DIR}/${BUILD_LIBRARY_DIR}/${BUILD_FLOAT} -L${TOOLCHAIN_DIR}/lib/gcc/${TOOLCHAIN_HOST}/${CMAKE_CXX_COMPILER_VERSION}/${BUILD_LIBRARY_DIR}/${BUILD_FLOAT} -Wl,-Map,${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${SOS_BSP_NAME}_${SOS_BSP_CONFIG}.map,--gc-sections${BUILD_HARDWARD_ID} -Ttext=${SOS_BSP_START_ADDRESS} -Tldscripts/${SOS_BSP_DEVICE}-rom.ld -nostdlib -u ${BUILD_UNDEFINE_SYMBOL} -u mcu_core_vector_table")

add_executable(${SOS_BSP_NAME}_${SOS_BSP_CONFIG}.elf ${SOS_BSP_SOURCELIST} StratifySettings.json StratifyLocalSettings.json)
add_custom_target(bin_${SOS_BSP_NAME}_${SOS_BSP_CONFIG} DEPENDS ${SOS_BSP_NAME}_${SOS_BSP_CONFIG}.elf COMMAND ${CMAKE_OBJCOPY} -j .text -j .data -O binary ${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${SOS_BSP_NAME}_${SOS_BSP_CONFIG}.elf ${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${SOS_BSP_NAME}.bin)
add_custom_target(asm_${SOS_BSP_NAME}_${SOS_BSP_CONFIG} DEPENDS bin_${SOS_BSP_NAME}_${SOS_BSP_CONFIG} COMMAND ${CMAKE_OBJDUMP} -S -j .text -j .priv_code -j .data -j .bss -j .sysmem -d ${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${SOS_BSP_NAME}_${SOS_BSP_CONFIG}.elf > ${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${SOS_BSP_NAME}.lst)
add_custom_target(size_${SOS_BSP_CONFIG} DEPENDS asm_${SOS_BSP_NAME}_${SOS_BSP_CONFIG} COMMAND ${CMAKE_SIZE} ${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${SOS_BSP_NAME}_${SOS_BSP_CONFIG}.elf)
add_custom_target(${SOS_BSP_CONFIG} ALL DEPENDS size_${SOS_BSP_CONFIG})


target_compile_definitions(${SOS_BSP_NAME}_${SOS_BSP_CONFIG}.elf PUBLIC __StratifyOS__ __${BUILD_TYPE} __${BULD_ARCH} __HARDWARE_ID=${SOS_BSP_HARDWARD_ID} __${SOS_BSP_DEVICE_FAMILY} __${SOS_BSP_DEVICE} ${SOS_BSP_DEFINITIONS})
target_link_libraries(${SOS_BSP_NAME}_${SOS_BSP_CONFIG}.elf ${BUILD_EXTRA_LIBRARIES} ${BUILD_LIBRARIES})
set_target_properties(${SOS_BSP_NAME}_${SOS_BSP_CONFIG}.elf PROPERTIES LINK_FLAGS ${LINKER_FLAGS})
target_compile_options(${SOS_BSP_NAME}_${SOS_BSP_CONFIG}.elf PUBLIC ${BUILD_FLAGS})
if(SOS_BSP_INCLUDE_DIRECTORIES)
	target_include_directories(${SOS_BSP_NAME}_${SOS_BSP_CONFIG}.elf PUBLIC ${SOS_BSP_INCLUDE_DIRECTORIES} )
endif()
